...
router.put('/update/:name', (req, res) => {
  Chef.updateOne({ name: req.params.name }, req.body).then(updatedChef => res.json(updatedChef));
});

router.delete('/delete/:name', (req, res) => {
  Chef.deleteOne({ name: req.params.name }).then(() => res.json({ message: 'Chef deleted' }));
});
