...
router.get('/chefs/:restaurantname', async (req, res) => {
  const restaurant = await Restaurant.findOne({ name: req.params.restaurantname }).populate('chefs');
  res.json(restaurant.chefs);
});

router.get('/recettes/:restaurantname', async (req, res) => {
  const restaurant = await Restaurant.findOne({ name: req.params.restaurantname }).populate('recettes');
...
router.put('/update/:name', (req, res) => {
  Restaurant.updateOne({ name: req.params.name }, req.body).then(updatedRestaurant => res.json(updatedRestaurant));
});

router.delete('/delete/:name', (req, res) => {
  Restaurant.deleteOne({ name: req.params.name }).then(() => res.json({ message: 'Restaurant deleted' }));
});
